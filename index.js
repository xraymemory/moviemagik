...
app.on("ready", () => {
    ...
    let mainWindow = new BrowserWindow({ height: 900, width: 1200, show: false, webPreferences: { nodeIntegration: true, additionalArguments: [conf_vlc_loc, root_dir_loc] } });
    ...
    ipcMain.on('load-idx', async function () {
        ...
        let files = dialog.showOpenDialog(mainWindow, { properties: ['openFile'] }).then((result) => {
            ...
            var allowedFiles = ['.csv', '.xml'];

            var allowed = allowedFiles.includes(path);
    ...
    ipcMain.on('reset-db', function () {

        knex.raw("delete from movies").then(function (result) {
    ...
    function readDir(fullPath) {
        ...
        fs.readdirSync(fullPath).forEach(file => {
            ...
            var allowedFiles = ['.mkv', '.m4a', '.mpg', '.mpeg4', '.avi', '.mov', '.mp4', '.divx', '.m4v'];
